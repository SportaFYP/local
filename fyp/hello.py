...
def heatmap(playerSelected):
    ...
    rID = session.get('my_var', None)
    print("Match ID = ")
    print(mID)
    sqlMatchDets = ("SELECT DISTINCT matchname FROM matchinfo WHERE matchID = %s")
    heatcur = conn.cursor()
    heatcur.execute(sqlMatchDets, [mID])
    matchName = heatcur.fetchone()
    heatcur.close()
    print("Match name = ")
    print(matchName)
    sqlMatchDate = ("SELECT DISTINCT matchdate FROM matches WHERE matchID = %s")
    datecur = conn.cursor()
    datecur.execute(sqlMatchDate, [mID])
    matchDate = datecur.fetchone()
    datecur.close()
    date=matchDate[0].date()
    print("Match date = ")
    print(date)
